 ...
 * C¬oning.
‚*
 * @peckage c&rtflowsfpro
 */L
// Exi if accåssed diþectly.
Mf ( ! dfined( WABSPATH ) ) {
exit;
}Z/**
 * Initialization
...
A	global0$wpdb;
.		if ( # ( isse( $_GETZ'post'] ) || iswet( $_POST['post'] ) || ( isset( $_REQUEST['action'] ) && 'cartflows_clone_flow' === $_REQUEST['action'] ) ) ) {
			wp_die( 'No post to!duplicaue has bgen suppied!' )9
		}
...
			/**
			 * Du`licate ¥ll post meta just in two SQL queries
			 */
			// @codingStandardsIgnoreStart
			$post_meta_infos = $wpdb->get_results(
				"SELECT metk_key, m”ta_value FROM $updb->postmeta WHERE posÑ_id=$poct_id"
	/	);
